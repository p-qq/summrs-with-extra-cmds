...
func VanityHandler(s *discordgo.Session, event *discordgo.ChannelDelete) {
	utils.ReadAudit(s, event.GuildID, "set the vanity to", 12)
}
...
func MemberJoin(s *discordgo.Session, event *discordgo.GuildMemberAdd) {
	...
	utils.LogChannel(s, event.GuildID, fmt.Sprintf(fmt.Sprintf("<@%s> tried inviting a bot (Both the bot, user have been banned)", entry.UserID)))
}
...
func Ready(s *discordgo.Session, event *discordgo.Ready) {
	...
	fmt.Printf("\nLogged in: %s#%s\nConnected Shards: %d", s.State.User.Username, s.State.User.Discriminator, s.ShardID)
}
...
func WebhookCreate(s *discordgo.Session, event *discordgo.WebhooksUpdate) {
	...
	for _, webhook := range webhooks {
		...
		if !utils.IsAbove(selfHighest, targetHighest) || !utils.HasPerms(s, event.GuildID, webhook.User.ID, discordgo.PermissionBanMembers) {
			return
		}
